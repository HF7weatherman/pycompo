        ...
        if not ofile_feat.exists():
            feature_props = \
                Parallel(n_jobs=config['parallel']['n_jobs_get_features'])(
                    delayed(process_one_timestep_get_props)(dsample, time, config)
                    for time in dsample['time']
                    )
            feature_props = pcsst.set_global_feature_id(feature_props)
            feature_props = xr.concat(feature_props, dim='feature')
            
            # Permut feature times
            ...
            features = \
                Parallel(n_jobs=config['parallel']['n_jobs_get_features'])(
                    delayed(process_one_timestep_safe)(dsample, feature_props, time, config)
                    for time in dsample['time']
                    )
...
def process_one_timestep_get_props(
        dset: xr.Dataset,
        time: np.datetime64,
        config: dict,
        ) -> xr.Dataset:
...
def process_one_timestep_safe(
    ...
    try:
        return process_one_timestep(dset, feature_props, time, config)
...
def process_one_timestep(
    ...
    data = dset.sel(time=slice(time, time))
    featprops = featprops.where(featprops['time']==time, drop=True)
